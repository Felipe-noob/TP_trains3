...
void build_request(xway_paquet_t paquet, uint8_t * requete) {
    ...
    requete[9] = (paquet.emetteur.reseau_id << 4) | (paquet.emetteur.porte_id & 0x0F);
    requete[10] = paquet.destinataire.station_id;
    requete[11] = (paquet.destinataire.reseau_id << 4) | (paquet.destinataire.porte_id & 0x0F);
    ...
    if (paquet.requete.segment_objet != 0) requete[req_off + 2] = paquet.requete.segment_objet;
    if (paquet.requete.type_objet != 0) requete[req_off + 3] = paquet.requete.type_objet;
    ...
    if (adresse_premier_mot != 0 || nb_mots != 0) {
