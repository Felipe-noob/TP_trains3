...
int main(int argc, char * argv[]) {
	...
	xway_address_t local = {EMETTEUR_STATION_ID, EMETTEUR_RESEAU_ID, EMETTEUR_PORT_ID};
	xway_address_t automate = {DESTINATAIRE_STATION_ID, DESTINATAIRE_RESEAU_ID, DESTINATAIRE_PORT_ID};
	...
	CHECKERROR(connect(sd1, (const struct sockaddr *)&addr_serv, sizeof(struct sockaddr_in)),-1, "Connexion fail !!!\n");
	...
	nbbytes=recvfrom(sd1,reponse, MAXOCTETS, 0, (struct sockaddr *)&addr_serv, &adr_len);
	if (nbbytes > 0) {
		printf("Reponse : \n");
